...
使用实际的RICE项目和论文指南进行完整的复现分析。

主要功能:
- 基于真实RICE项目代码的分析
- 使用paper_test_1_reproduction_guide.md作为指南
...
def create_rice_target_metrics():
    """
    创建RICE论文复现的目标指标
    
    返回一个简单的字符串描述，告诉系统我们想要完美复现论文。
    """
    return "完美复现RICE论文的所有算法、实验和结果，确保与论文描述完全一致"


def run_rice_paper_reproduction():
    """运行RICE论文的完整复现流程（包含分析和执行）"""
    ...
    # 配置参数
    config = {
        "repo_path": "/Users/wwchdemac/python_projects/debug_agent/test_input/rice",
        ...
        "target_metrics": create_rice_target_metrics(),
    ...
    if valid_additional_guides:
        ...
        print(f"📚 补充信息文档: 未提供")
    print(f"🎯 目标指标类别: {len(config['target_metrics'])}个主要类别")
...
def show_target_metrics_summary():
    """显示目标指标摘要"""
    metrics = create_rice_target_metrics()
    
    print("\n🎯 RICE论文复现目标:")
    print("-" * 40)
    print(f"📌 {metrics}")
    print("\n💡 提示: 您可以在 create_rice_target_metrics() 函数中编辑目标描述")


def run_rice_with_additional_guides_example():
    """
    展示如何使用补充信息功能的示例
    ...
    try:
        ...
        result = reproduction_system.reproduce_paper(
            repo_path=config["repo_path"],
            paper_guide=config["paper_guide"],
            additional_guides=config["additional_guides"],
            target_metrics=config["target_metrics"],
            max_iterations=config["max_iterations"]
        )
...
if __name__ == "__main__":
    ...
    if choice == "1":
        ...
        # 运行完整复现流程
        success = run_rice_paper_reproduction()
        ...
        print("\n📚 展示补充信息功能...")
        run_rice_with_additional_guides_example()
        print("\n💡 如需实际使用补充信息功能，请:")
        print("   1. 创建您的补充信息markdown文档")
        print("   2. 在 run_rice_with_additional_guides_example() 函数中配置路径")
        print("   3. 重新运行此选项")
